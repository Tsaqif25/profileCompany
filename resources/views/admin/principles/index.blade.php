                    ...
                    @foreach($principle as $item)
                        <div class="item-card flex flex-row justify-between items-center">
                            <div class="flex flex-row items-center gap-x-3">
                                <img src="{{ asset('storage/'.$item->thumbnail) }}" alt="{{ $item->name }}" class="rounded-2xl object-cover w-[90px] h-[90px]">
                            ...
                            <div class="hidden md:flex flex-row items-center gap-x-3">
                                <a href="{{ route('admin.principles.edit', $item->id) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                                    Edit
                                </a>
                                <form action="{{ route('admin.principles.destroy', $item->id) }}" method="POST"> 
                                    @csrf
                                    @method('DELETE')
