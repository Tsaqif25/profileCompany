...
@endif
    ...
    <form action="{{ route('front.appointement_store') }}" method="POST" class="flex flex-col p-[30px] rounded-[20px] gap-[18px] bg-white shadow-[0_10px_30px_0_#D1D4DF40] w-full md:w-[700px] shrink-0">
       @csrf
      ...
      <div class="flex items-center gap-[18px]">
        <div class=" flex-col gap-2 flex w-full">
          ...
          <div class="flex items-center gap-[10px] p-[14px_20px] border border-[#E8EAF2] focus-within:border-cp-dark-blue transition-all duration-300 rounded-xl bg-white">
            ...
            <input type="tel" name="phone_number" id="" class="appearance-none outline-none bg-white placeholder:font-normal placeholder:text-cp-black font-semibold w-full" placeholder="exp 6285454" required>
          </div>
        </div>
        <div class="flex-col gap-2 flex w-full">
        ...
        <script>
            // Javascript untuk men-trigger input type date saat tombol diklik
            document.getElementById('dateButton').addEventListener('click', function() {
                document.getElementById('dateInput').focus(); // Trigger input type="date"
            });
        
            // Menampilkan tanggal yang dipilih pada tombol setelah input dipilih
            document.getElementById('dateInput').addEventListener('change', function() {
                document.getElementById('dateButton').textContent = this.value; // Tampilkan tanggal yang dipilih
            });
        </script>
    ...
    <div class="main-carousel w-full">
      ...
      <div class="carousel-card container max-w-[1130px] w-full flex flex-wrap justify-between items-center lg:mx-[calc((100vw-1130px)/2)]">
        ...
        <div class="testimonial-thumbnail w-[470px] h-[550px] rounded-[20px] overflow-hidden bg-[#D9D9D9]">
          <img src="{{ Storage::url($testimonial->thumbnail) }}" class="object-contain" alt="icon">
        </div>
      </div>
      @empty
